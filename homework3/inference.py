...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            # get new distribution given game state
            new_dist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            # get past probability 
            old_prob = self.beliefs[p]
            for item in new_dist.keys():
                allPossible[item] += old_prob * new_dist[item]
          
        # set beliefs equal to new distribution
        allPossible.normalize()
        self.beliefs = allPossible
